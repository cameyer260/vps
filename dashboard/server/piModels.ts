 ...
 * (get_available_models). Runs `jarvis models` (an ephemeral agent-pi
 * container's `pi --list-models`) and caches the result — the catalog only
 * changes on pi updates / provider catalog refreshes.
 *
 * pi ships only in the agent-pi image; jarvis is the single source of truth
 * for the version (see agent-images/jarvis.sh). When jarvis/the image is
 * missing the route fails and the picker stays on the scoped list.
 */
...
export async function listAllModels(): Promise<AllModel[]> {
  ...
  const output = await new Promise<string>((resolve, reject) => {
    execFile(
      ...
      (err, stdout, stderr) => {
        if (err) {
          reject(new Error(`jarvis models failed: ${String(stderr || err.message).trim().slice(-300)}`));
        } else {
          resolve(stdout);
        }
      },
  ...
  if (models.length === 0) throw new Error("jarvis models returned no models");
